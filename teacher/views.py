...
def teacher_appointment_list(request):
	...
	if "Teacher" == group_name:
		...
		if form.is_valid():	
			toAdd = True
			#temp_date = parse_date(form['date'].value())
			#date_str = form['date'].value()
			#print(date_str)
			#temp_date = datetime.strptime(date_str, "%d/%m/%Y").date()
			#print(temp_date)
			#print(form['time_start'].value())
			#print(form['time_end'].value())
			#print(datetime.date.today())
			#print(datetime.datetime.now().time())

			#print(datetime.strptime(form['date'].value(), '%d/%m/%Y').date()) #dd/mm/yyyy
			#current_datetime = datetime.datetime.now()  
			#current_datetime = datetime.date.today()
			#if form['date'].value() < str(current_datetime):
				# | (form['date'].value() == str(datetime.date.today()) & form['time_start'].value()<datetime.datetime.now().time()):
				#toAdd = False
				#messages.error(request, 'Cant add appointments in the past')
				#return redirect('http://127.0.0.1:8000')
			#else:
				#toAdd = True
				#print("false")
