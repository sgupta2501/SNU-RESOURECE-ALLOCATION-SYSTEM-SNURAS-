...
def appointment_book(request, id):#activate after clicking book now button
	...
	if "Student" == group_name:
		...
		#print(single_appointment.time_start)
		#print(single_appointment.date)
		#print(single_appointment.time_end)			
		toAdd = True
		for a in appointment_list:
			#print(a.time_start)
			#print(a.date)
			#print(a.time_end)			
			if (single_appointment.date == a.date):
				if (single_appointment.time_start >= a.time_end) | (single_appointment.time_end <= a.time_start):
					#print('true')
					toAdd = True
				else:
					#print("false")
